...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while True:
        print("\nYour hand: ", end="")
        display_hand(hand)
        word = input("Enter word, or '!!' to finished game: ")
        ...
        if calculate_handlen(hand) == 0:
            print("\nThe letters are over. Total score: ", total_score," points.\n----------------------")
            break
    return total_score
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    points = 0
    is_letter_replaced = False
    numhand = int(input("How many hands did you want to hand out? : "))

    while numhand != 0:
        hand = deal_hand(HAND_SIZE)
        print("Your hand: ", end=" ")
        display_hand(hand)
        if not is_letter_replaced:
            if input("If you want replace 1 letter input 'yes' or if no - anything else):") == "yes":
                letter = str(input("What letter you want change? "))
                hand = substitute_hand(hand, letter)
                is_letter_replaced = True
